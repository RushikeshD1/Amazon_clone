...
const elecHeader = 
    ["Electronic","Mobiles & Accessories","TV & Home Entertainment","Audio","Gaming Accessories"]
...
const Electronic = () => {
    ...
    const renderProduct = () => {
        ...
        const filterProduct = product.filter((product) => product.category !== 'mobile' && product.category !== 'tv')
  ...
  return (
    <div className="w-full  ">
        <Header />
        <div className=" flex flex-row items-center justify-start h-14 gap-4 pl-8 bg-white w-full">
            <div className="text-sm font-bold cursor-pointer">{elecHeader[0]}</div>
            {elecHeader.slice(1).map((items, index) =>             
                <div className="text-[12px] cursor-pointer relative group sm:block hidden" key={index}>
                    {items}                                
                ...
                <span className="fixed left-0 hidden group-hover:block bg-transparent w-screen">
                    <div className="h-4 w-4 translate-x-[170px] translate-y-1/2 rotate-45 bg-white border-l border-t border-gray-200"></div>
                ...
                {items === "Audio" &&                   
                <span className="fixed left-0 hidden group-hover:block bg-transparent w-screen">
                    <div className="h-4 w-4 translate-x-[410px] translate-y-1/2 rotate-45 bg-white border-l border-t border-gray-200"></div> 
                    <FlyoutLink FlyoutContent={AudioAccess} elecHeader={elecHeader[0]} />
                </span>}
                {items === "Gaming Accessories" &&                   
                <span className="fixed left-0 hidden group-hover:block bg-transparent w-screen">
                    <div className="h-4 w-4 translate-x-[500px] translate-y-1/2 rotate-45 bg-white border-l border-t border-gray-200"></div>
                    <FlyoutLink FlyoutContent={GamingAccess} elecHeader={elecHeader[0]} />
                </span>}
                {items === "Electronic" &&                   
                <span className="fixed left-0 hidden group-hover:block bg-transparent w-screen">
                    <div className="h-4 w-4 translate-x-40 translate-y-1/2 rotate-45 bg-white border-l border-t border-gray-200"></div>
